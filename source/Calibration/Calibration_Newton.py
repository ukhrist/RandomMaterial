...
from hessian import gradient, hessian
...
class calibrate():
    ...
    def __call__(self, MaterialModel, Data, **kwargs):
        ...
        for n_iter in range(max_iter):
            ...
            Batch = self.draw_batch(self.batch_size)

            batch_vals, batch_grad, batch_hess = [], []
            for w in Batch:
                self.Optimizer.zero_grad()
                self.Model.update()
                val = self.Model.forward(w)
                grad = gradient(val, params)
                hess = hessian(val, params)
                batch_vals.append(val)
                batch_grad.append(grad)
                batch_hess.append(hess)

            if self.update_batch_size(batch_grad, batch_hess):
            ...
            if self.optim_method == 'SGD':
                p = -lr*g_Sk
            elif self.optim_method == 'Newton':
                p = self.get_direction(H, g)
    ...
    def update_batch_size(self, grads):
        ...
        print('\n New batch size  = {0:d}'.format(self.batch_size)))
